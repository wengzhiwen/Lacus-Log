...
## 2025-09-28 新增：
- 开播日报邮件发送功能：新增开播日报邮件发送功能，每天0:02自动发送前一自然日的开播日报邮件给全体运营和管理员，邮件包含本月数据、日报汇总、明细数据三个板块，明细数据已简化去掉部分列以适配邮件格式
- 开播日报手动触发功能：在邮件报告页面新增开播日报手动触发按钮，支持管理员手动发送指定日期的开播日报邮件，便于调试和测试
