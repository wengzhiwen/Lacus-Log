...
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_get_pilots_list_success | 获取列表 - 成功 | ⏳ | P0 | 基础功能 |
| test_get_pilots_list_with_pagination | 分页查询 | ⏳ | P0 | 验证分页 |
| test_get_pilots_list_with_filters | 多条件过滤 | ⏳ | P1 | rank/status/platform等 |
| test_get_pilots_list_by_owner | 按运营筛选 | ⏳ | P1 | 权限相关 |
| test_get_pilots_list_unauthorized | 未登录访问 | ⏳ | P2 | 权限测试 |

#### 2.2 TestPilotsCreate - 创建主播
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_create_pilot_success | 创建主播 - 成功 | ⏳ | P0 | 完整数据 |
| test_create_pilot_minimal_data | 最小必需数据 | ⏳ | P0 | 仅必填字段 |
| test_create_pilot_duplicate_check | 重复检测 | ⏳ | P1 | 昵称/电话重复 |
| test_create_pilot_invalid_rank | 无效rank | ⏳ | P2 | 数据验证 |
| test_create_pilot_as_kancho | 运营创建主播 | ⏳ | P1 | 自动关联owner |

#### 2.3 TestPilotsDetail - 主播详情
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_get_pilot_detail_success | 获取详情 - 成功 | ⏳ | P0 | 完整信息 |
| test_get_pilot_detail_not_found | 主播不存在 - 404 | ⏳ | P1 | 错误处理 |
| test_get_pilot_detail_permission | 运营查看权限 | ⏳ | P1 | 只能查看自己的 |

#### 2.4 TestPilotsUpdate - 更新主播
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_update_pilot_success | 更新信息 - 成功 | ⏳ | P0 | 基础字段 |
| test_update_pilot_rank | 更新等级 | ⏳ | P1 | rank变更 |
| test_update_pilot_owner | 转移主播 | ⏳ | P1 | owner变更（仅管理员） |
| test_update_pilot_not_found | 主播不存在 - 404 | ⏳ | P1 | 错误处理 |
| test_update_pilot_permission | 运营更新权限 | ⏳ | P1 | 只能更新自己的 |

#### 2.5 TestPilotsDelete - 删除主播
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_delete_pilot_success | 删除主播 - 成功 | ⏳ | P0 | 软删除 |
| test_delete_pilot_with_records | 有开播记录的主播 | ⏳ | P1 | 业务规则 |
| test_delete_pilot_permission | 运营删除权限 | ⏳ | P1 | 权限控制 |

#### 2.6 TestPilotsConflict - 冲突检测
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_check_conflict_exists | 检测到冲突 | ⏳ | P1 | 昵称/电话冲突 |
| test_check_conflict_none | 无冲突 | ⏳ | P1 | 新数据 |

#### 2.7 TestPilotsWorkflow - 完整流程
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_pilot_recruitment_flow | 招募流程 | ⏳ | P2 | 候选人->签约->正式 |
| test_pilot_lifecycle | 完整生命周期 | ⏳ | P2 | 创建->更新->删除 |
...
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_complete_recruitment_flow | 完整招募流程 | ⏳ | P1 | 创建用户->创建主播->招募记录 |

#### 9.2 TestAnnouncementWorkflow - 通告工作流
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
| test_announcement_to_record_flow | 通告转开播记录 | ⏳ | P1 | 创建通告->创建开播记录 |

#### 9.3 TestPermissionWorkflow - 权限工作流
| 测试用例 | 场景 | 状态 | 优先级 | 备注 |
|---------|------|------|--------|------|
