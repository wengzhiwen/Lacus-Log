...
## 2025-11-01 新增：
- 主播重名检查功能：在新建和编辑主播页面，当用户修改主播昵称或真实姓名时，系统会自动检查是否存在重名的其他主播。如发现重名，在输入框上方显示醒目的警告信息，包含重名主播的昵称、真实姓名、年龄、性别、直属运营和最后更新时间。提交时如存在重名会弹出二次确认浮层，确保用户知晓重名情况。

## 2025-10-30 新增：
- 助理运营(gunsou)角色：新增助理运营角色，权限为运营角色的真子集，支持在用户管理中进行角色切换

## 2025-10-29 新增：
- 主播业绩页招募负责人显示：主播业绩页头部信息中新增招募负责人字段，显示该主播关联的最后更新的招募信息的招募负责人，若无招募记录则显示"--"。
- S10报表筛选功能测试套件：新增完整的报表筛选功能集成测试套件，包含12个测试用例，全面验证日报、周报、月报和底薪月报的mode、owner、settlement筛选功能，确保筛选逻辑正确性和数据一致性。
- 底薪月报功能：新增完整的底薪月报功能，支持按月统计当月所有开播记录及其关联的底薪申请情况。

## 2025-10-30 修复：
- 用户登录IP地址获取错误：修复用户登录时系统记录的IP地址为代理服务器IP（127.0.0.1）而非真实客户端IP的问题。创建统一的IP获取工具函数`utils/request_helper.py`，优先从`X-Forwarded-For`请求头获取真实客户端IP，在反向代理环境下能正确记录用户真实IP地址。影响范围包括认证模块、结算模块、开播记录模块和底薪申请模块的日志记录和操作追踪。
- 主播业绩页返回按钮路径错误：修复从快速月报进入主播业绩页后返回按钮404的问题，更正了所有报表页面的返回路径（日报/new-reports/daily、周报/new-reports/weekly、快速月报/new-reports-fast/monthly），同时支持更多来源页面的正确返回（底薪申请详情、通告详情等）。
- 底薪月报筛选逻辑错误：修复底薪月报中"全部"结算方式筛选条件只显示无底薪记录的严重逻辑错误，修正后"全部"筛选能正确显示所有类型记录（月结底薪+日结底薪+无底薪），确保数据完整性和筛选功能准确性。
- 开播记录详情页主播信息显示错误：修复开播记录详情页中同样存在的主播昵称显示为年龄的问题，将显示格式从"主播昵称(年龄)"修正为"主播昵称(真实姓名)"，确保详情页信息展示的准确性。
- 开播记录列表页卡片格式同步通告列表：优化开播记录列表页的数据卡片上半部分显示格式，与通告列表页保持一致的布局和样式。主播信息移至左侧并采用分层展示（昵称+详情行），时间信息移至右侧，保留开播状态指示器功能，提升界面一致性和信息可读性。
- 开播月报邮件明细数据缺失：修复2025-10-26重构报表系统时引入的错误，恢复开播月报邮件中的主播明细数据。问题原因是错误地认为快速月报没有明细计算函数，但实际上`calculate_monthly_details_fast`函数存在且可用。

## 2025-10-29 优化：
- 开播报表主播链接状态显示：优化日报、周报、月报明细列表中的主播链接显示，当主播状态为流失时，链接自动变为灰色样式（#6c757d），保持链接功能不变但视觉效果更直观，帮助运营快速识别流失主播。

## 2025-10-29 新增：
- 底薪发放提醒邮件功能：新增定时邮件提醒功能，每天18:00自动检查状态为"未处理"且申请时间已超过12小时的底薪申请，向管理员和运营发送提醒邮件，邮件内容包含主播信息、开播日期、申请时间和超时小时数，确保及时处理超时底薪申请。
- 底薪发放提醒管理界面：在邮件报告页面新增底薪发放提醒卡片，支持手动触发邮件发送，显示下次自动触发时间，并提供功能说明，方便管理员灵活管理底薪发放提醒。
- 底薪发放提醒测试覆盖：新增S9-TC5测试用例，验证底薪发放提醒邮件的生成逻辑和内容格式，确保在MAIL_DEBUG模式下能正确生成邮件文件，内容包含必要的表格结构和提醒信息。

## 2025-10-28 修复：
- CSV导出缓存问题解决：为所有报表（日报、周报、快速月报）的CSV导出功能添加动态时间戳URL参数，避免CDN缓存导致数据不一致问题。通过JavaScript生成带时间戳的唯一请求URL，确保每次导出都获取最新数据。

## 2025-10-28 新增：
- 快速月报明细表新增日均毛利字段：在月累计流水后增加日均毛利（元）字段，计算方式为月累计毛利除以有效开播天数（播时≥1小时的天数），帮助运营更直观评估主播日均盈利能力。
- 开播周报性能优化：参考快速月报架构，实现开播新周报（加速版）计算模块，采用单次数据库查询、批量预取分成比例、缓存机制等优化策略，显著提升周报加载性能，同时保持计算结果与原周报完全一致。
- 开播周报前一周对比功能：新增前一周开播记录数、前一周均播时、前一周累积流水、前一周累积毛利4个对比字段，支持周环比分析，提升运营决策支持能力。采用两周数据一次查询优化，确保高性能计算。
- 开播周报明细表前端排序功能：参考快速月报实现，为周报明细表添加纯前端排序功能，支持所有15个字段的双向排序，包含排序指示器和键盘无障碍支持，提升数据分析效率。

## 2025-10-28 优化：
- 返点计算逻辑统一与优化：创建统一的返点计算模块 `utils/rebate_calculator.py`，整合返点阶梯设置和计算逻辑，供快速月报、传统报告和主播业绩页共同使用，消除代码重复，确保计算逻辑一致性。
- 返点阶梯设置更新：按照文档要求调整返点阶梯配置，降低门槛门槛（10天/30小时/1000元起），调整返点比例（5%-16%五级阶梯），提升主播获得返点的机会和激励效果。

## 2025-10-27 新增：
- 快速月报增加按主播状态筛选功能：支持按未招募、不招募、已招募、已签约、流失状态筛选报表数据，默认为全部状态，筛选基于主播当前状态而非开播当时状态。

## 2025-10-26 重构：
- 精简开播报表系统：移除传统日报、月报、周报及新月报功能，保留新日报、新周报、快速月报三个核心报表类型，删除相关模板、API接口和计算逻辑，优化系统结构和维护成本。

## 2025-10-26 修复：
- 修复报表系统精简后的导入依赖问题：调整 report_mail.py 和相关序列化模块，确保快速月报功能正常工作，恢复必要的月报序列化函数以支持邮件报表功能。
- 修复模板中的路由引用错误：移除 base.html 和 dashboard/index.html 中对已删除路由的引用，更新相关文档中的链接指向保留的报表功能，确保页面正常渲染和测试通过。
