...
## 2025-10-16 修复：
- 开播日报API JSON序列化错误：修复状态枚举未被正确转换为字符串导致的JSON序列化失败问题，确保 `record.current_status.value` 直接转换为字符串值。本修复需要重启Flask服务以清除缓存中的旧数据
- 开播日报明细状态显示修复：修复前端状态显示，确保状态为"ended"或为空时都显示为"已下播"而不是原始英文值
