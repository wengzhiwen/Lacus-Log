...
## 2025-10-09 修复：
- **历史数据兼容性修复**：修复历史数据缺少中间决策时间的问题，确保新开播数和满7天数计算不受影响：
  - 统计逻辑保持不变，仍基于决策时间计算（新开播数基于broadcast_decision_time，满7天数基于被决定招募的主播）
  - 明细数据改为展示窗口内创建的所有招募记录，包含完整招募流程
  - 这样可以处理历史数据中某些招募记录缺少面试决策时间的情况，确保数据的完整性和准确性
  - 满足用户要求：由于历史原因部分数据面试决策时间不存在，但这些数据不应该影响新开播数和满7天数的计算
