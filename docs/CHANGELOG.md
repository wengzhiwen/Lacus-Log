...
## 2025-10-09 新增：
- **主播招募月报功能模块开发**：完成主播招募月报功能的全套开发，包括：
  - **统计工具扩展**：扩展utils/recruit_stats.py，新增月报统计、转化率计算、满7天数统计、趋势比较等核心功能
  - **序列化工具**：创建utils/recruit_monthly_report_serializers.py，提供月报数据的统一序列化格式
  - **REST API**：创建routes/recruit_monthly_reports_api.py，提供月报汇总和明细数据的REST接口
  - **页面路由**：创建routes/recruit_monthly_reports.py，提供月报页面访问入口
  - **模板页面**：创建templates/recruit_reports/monthly.html，实现响应式月报界面
  - **菜单集成**：在主导航中添加"主播招募月报"入口，位于招募日报下方
  - **核心指标**：约面数、到面数、试播数、新开播数、满7天数，包含转化率计算和趋势比较
  - **明细功能**：支持按招募负责人筛选，展示招募明细卡片，包含主播搜索过滤
  - **时间口径**：滚动30天窗口，支持与上期数据对比的趋势分析
  - **UI风格统一**：页面上半部分设计与开播日报保持一致，包括loading状态、汇总卡片动画效果和视觉样式
  - **Loading状态修复**：移除多余的统计窗口文字提示，统一使用公共loading图标，确保数据加载完成后提示正确消失
  - **明细卡片内容重构**：按照业务需求重新组织明细卡片内容结构，包括：
    - 主播信息：移除假头像，昵称（真实姓名）性别图标的一行显示，使用开播记录详情页的性别图标格式（♂/♀）
    - 招募状态、渠道、介绍费、直属运营、开播天数（加粗）、最近开播时间的完整信息展示
    - 关键日期：所有时间统一为GMT+8日期格式（月-日），独立显示启动招募、面试决策、开播决策日期
  - **字段显示修复**：修正明细卡片中"直属运营"字段的显示，确保运营人员信息正确展示
  - **字段布局优化**：明细卡片中同时显示"招募负责人"和"直属运营"字段，并将"最近开播"移至底部日期区域与其他关键日期一起展示
  - **日期格式化修复**：修复明细卡片中显示"Invalid Date"的问题，改进`formatDateOnly`函数的错误处理和时区转换逻辑
  - **API数据字段映射修复**：修复介绍费字段缺失和决策日期显示异常问题，将日期字段从显示格式改为ISO格式返回，确保前端能正确解析和显示日期
  - **统计窗口扩展**：将招募月报的滚动统计窗口从30天扩展到60天，提供更长周期的招募数据分析视角，包括：
    - 当期窗口：今天向前滚动60天（包含今天）
    - 对比窗口：第61-120天
    - 明细数据范围同样扩展到60天窗口
    - 满7天计算逻辑调整为基于60天窗口内开播记录
  - **前端显示文本统一**：修复前端模板和文档中所有30天的描述文本，统一更新为60天，确保用户界面与实际统计逻辑保持一致
